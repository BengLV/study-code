...
public class CommonInterviewTopic {
   ...
   /* public class Solution {
     ...
     */
    public 算法.链表.ListNode mergeKLists(ArrayList<算法.链表.ListNode> lists) {
        if (lists == null || lists.size() == 0) return null;
        //使用有线队列
        PriorityQueue<算法.链表.ListNode> deque = new PriorityQueue<>(lists.size(), (o1, o2) -> {
            if (o1.val < o2.val) {
                return -1;
        ...
        算法.链表.ListNode dummy = new 算法.链表.ListNode(0);
        算法.链表.ListNode tail = dummy;
        //先添加头部节点即可.
        for (算法.链表.ListNode node : lists) {
            if (node != null) {
                deque.add(node);
            }
        }
